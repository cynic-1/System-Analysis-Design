<template xmlns="http://www.w3.org/1999/html">
  <div class="q-pa-md" style="margin-left: 20%">
    <div class="q-gutter-y-md column" style="width: 80%; max-width: 100%">
      <q-toolbar class="bg-grey-2 shadow-2 rounded-borders" style="height: 60px">
        <q-btn-dropdown :label="searchby" color="white"  style="width: 15%; height: 60%" text-color="black">
          <q-list>
            <q-item clickable v-close-popup v-for="item in list" :key="item" @click="onItemClick(item)">
              <q-item-section>
        ...
        <q-input dark dense standout v-model="key" input-class="text-left" class="q-ml-md" style="width: 80%" bg-color="white">
          <template v-slot:append>
            <q-separator vertical inset />
            <q-icon name="search" class="cursor-pointer" @click="key = ''" />
          </template>
        </q-input>
        <q-separator vertical inset style="margin-left: 10px"/>
        <q-btn color="grey-3" text-color="black" flat stretch style="margin-left: 10px" no-wrap>结果中检索</q-btn>
        <q-separator vertical inset style="margin-left: 10px"/>
        <q-btn color="grey-3" text-color="black" flat stretch style="margin-left: 10px" no-wrap
          @click="advanced">高级检索</q-btn>
      </q-toolbar>
    ...
    <div class="q-gutter-y-md" style="width: 100%" >
      ...
      >
        <span v-for="item in categories" :key="item">
          <q-tab v-if="item.selection.length ===0" :name="item.name" :label="item.name + '( ' + item.num + ' )'"/>

          <q-btn-dropdown v-else auto-close stretch flat :label="item.name">
            <q-list>
              <q-item v-for="opt in item.selection" :key="opt" clickable @click="tab = ''">
  ...
  <div class="row bg-white" style="min-height: 400px; width: 92%; padding: 24px;margin-left: 8%">
    <div id="parent" class="fit row wrap justify-start items-start content-start" style="overflow: hidden;">
      <div  class="q-pa-md" style="overflow: auto;width: 18%">
        <q-card class="my-card text-blue" flat bordered>
          <q-card-section horizontal>
            <q-card-actions vertical align="center">
              <q-btn flat size="20px" style="margin-left: 15px">
            ...
            <q-card-actions vertical style="margin-left: 15px">
              <q-btn flat >中文</q-btn>
              <q-btn flat >外文</q-btn>
            </q-card-actions>
          </q-card-section>
          ...
          <q-card-actions align="around">
            <q-btn color="grey-3" text-color="blue" size="18px">科技</q-btn>
            <q-btn color="grey-3" text-color="blue" size="18px">社科</q-btn>
          </q-card-actions>
        </q-card>
        ...
        <q-card class="my-card" style="margin-top: 10px">
          <q-card-section class="q-pt-none" style="background-color: #cdcdcd; font-weight: bold; font-size: medium; color: #4c4c4c; height: 28px">
            为我推荐
          </q-card-section>
          <q-card-section>
      ...
      <div class="q-pa-md" style="width: 72%; margin-left: 10px">
        <q-table
          :rows="rows"
          :columns="columns"
          row-key="name"
        ...
        <div style="margin-top: 25px">
          ...
          <span v-for="item in related" :key="item" style="margin-right: 25px;color: #006cd6; cursor: pointer;">{{item}}</span>
        </div>
